...
var ensCtrl = function ($scope, $sce, walletService) {
  ...
  $scope.openAndBidAuction = function () {
    ...
    getShaBid($scope.bidObject, function (isError, data) {
      if (isError) {
        ...
        $scope.tx.data = ENS.getStartAndBidAuctionData($scope.objENS.name, data);
  ...
  $scope.generateTx = function () {
    try {
      ...
      $scope.sentTxs = [];
      $scope.generatedTxs = [];
      if (!$scope.Validator.isValidENSName(_objENS.name)) {
        throw globalFuncs.errorMsgs[30];
      }

      if (!$scope.Validator.isPositiveNumber(_objENS.bidValue) || _objENS.bidValue < 0.01 || _objENS.status !== $scope.ensModes.reveal && (!$scope.Validator.isPositiveNumber(_objENS.dValue) || _objENS.dValue < _objENS.bidValue || $scope.wallet.balance <= _objENS.dValue)) throw globalFuncs.errorMsgs[0]; else if (!$scope.Validator.isPasswordLenValid(_objENS.secret, 0)) throw globalFuncs.errorMsgs[31];
      ...
      else if ($scope.objENS.status === $scope.ensModes.open) $scope.openAndBidAuction();
